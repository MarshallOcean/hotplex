...
- **`chatapps/`**：**平台接入层**。将 HotPlex 连接到社交平台，以 **Slack** (基于 Block Kit) 为核心示例，同时也支持 Discord, Telegram 等。
...
```mermaid
sequenceDiagram
    participant Social as "Slack / 社交平台"
